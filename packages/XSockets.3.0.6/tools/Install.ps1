...
if($webproject -eq $true){
    ...
    Write-Host "If you are new to the XSockets.NET consider installing the package XSockets.Sample.Stockticker or XSockets.Sample.WebRTC" -ForegroundColor Blue
    Write-Host "Install-Package XSockets.Sample.Stockticker" -ForegroundColor Blue
    Write-Host "Install-Package XSockets.Sample.WebRTC" -ForegroundColor Blue
    Write-Host "########################################################################################################################" -ForegroundColor Green
}
...
Write-Host "using (var server = XSockets.Plugin.Framework.Composable.GetExport<IXSocketServerContainer>())" 
Write-Host "{"
